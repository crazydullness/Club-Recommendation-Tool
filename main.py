...
class ImageCarouselWidget(QWidget):
    def __init__(self, image_paths, interval=3000, parent=None):
        ...
        self.label.setFixedSize(883, 500)  # 设置为固定轮播尺寸（可根据图片原始比例调整）
...
class ClubRecommendationApp(QMainWindow):
    ...
    def initUI(self):
        ...
        carousel_container_layout = QHBoxLayout()
        carousel_container_layout.setContentsMargins(0, 20, 0, 10)
        carousel_container_layout.setAlignment(Qt.AlignCenter)
        carousel_container.setLayout(carousel_container_layout)
        carousel_container_layout.addWidget(self.carousel)
        main_layout.addWidget(carousel_container)

        # 标题打字动画
        self.title = QLabel("")
        self.title.setFont(QFont("微软雅黑", 28, QFont.Bold))
        self.title.setStyleSheet("color: #222222; margin-top: 10px;")
